 ...
 *  - 「meta」シートに分類・口座・予算・定期収支などを JSON で保存
 *  - 端末から「前回同期以降に変わった明細」と「消した明細のID」を受け取り、
 *    サーバー側で前回同期以降に他の端末が変更したものは他端末の変更を優先する
 *  - 端末には「前回同期以降にサーバーで変わった明細」を返す
 */

const COLS = ['id', 'date', 'type', 'category', 'item', 'amount', 'memo', 'account', 'toAccount', 'store', 'receiptId', 'ruleId', 'autoPay', 'updatedAt', 'deleted'];
...
function doPost(e) {
  ...
  try {
    ...
    const metaSheet = getSheet_(ss, 'meta', ['key', 'value']);
    const now = Date.now();
    ...
    values.forEach(function (v) {
      const o = {}; COLS.forEach(function (c, i) { o[c] = v[i]; });
    ...
    (body.entries || []).forEach(function (ce) {
      ...
      const o = { id: id, date: String(ce.date || ''), type: String(ce.type || 'expense'), category: String(ce.category || ''), item: String(ce.item || ''),
        ...
        store: String(ce.store || ''), receiptId: String(ce.receiptId || ''), ruleId: String(ce.ruleId || ''), autoPay: String(ce.autoPay || ''), updatedAt: now, deleted: 0 };
    ...
    if (changedSheet) {
      ...
      if (out.length) sheet.getRange(2, 1, out.length, COLS.length).setValues(out);
    }
    ...
    let metaOut = null;
    if (body.meta) {
      if (meta.updatedAt > since && meta.json) metaOut = JSON.parse(meta.json); // 他端末が先に変更
      else writeMeta_(metaSheet, JSON.stringify(body.meta), now);
    } else if (meta.updatedAt > since && meta.json) {
      metaOut = JSON.parse(meta.json);
    }
    ...
    return out_({ ok: true, serverTime: now, entries: entries, deleted: deleted, meta: metaOut });
  } catch (err) {
...
function readMeta_(sh) {
  ...
  const res = { json: '', updatedAt: 0 };
  if (n < 2) return res;
  sh.getRange(2, 1, n - 1, 2).getValues().forEach(function (r) {
    if (r[0] === 'meta') res.json = String(r[1] || '');
    if (r[0] === 'updatedAt') res.updatedAt = Number(r[1]) || 0;
  });
  return res;
}
function writeMeta_(sh, json, now) {
  sh.getRange(2, 1, 2, 2).setValues([['meta', json], ['updatedAt', now]]);
}
function out_(obj) {
